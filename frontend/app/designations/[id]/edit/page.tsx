...
export default function DesignationForm() {
    ...
    const { designations, createDesignation, updateDesignation } = useDesignations();
    const [formData, setFormData] = useState({ designation_name: '', description: '', status: 'active' as 'active' | 'inactive' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setLoading(true);
        const result = isEdit && params?.id ? await updateDesignation(parseInt(params.id as string), formData) : await createDesignation(formData);
        if (result.success) {
            ...
            <main className="max-w-3xl mx-auto px-4 py-8">
                <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-4">
                    <div>
                        ...
                        <input required value={formData.designation_name} onChange={e => setFormData({ ...formData, designation_name: e.target.value })} className="w-full px-4 py-2 border rounded-lg" />
                    </div>
                    <div>
                    ...
                    <div className="flex gap-4 pt-4">
                        <Link href="/designations" className="px-6 py-2 border rounded-lg">Cancel</Link>
