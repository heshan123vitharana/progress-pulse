            ...
            <div className="p-6 lg:p-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 className="text-2xl lg:text-3xl font-bold text-slate-800">Projects</h1>
                            <p className="text-slate-500 text-sm">{filtered.length} of {projects.length} projects</p>
                        </div>
                    </div>
                    <Link
                        href="/projects/create"
                        className="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium rounded-xl hover:from-blue-600 hover:to-indigo-700 shadow-lg shadow-blue-500/30 hover:shadow-xl transition-all"
                    >
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-2.5 border border-slate-200 rounded-xl text-slate-700 bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                            />
                        </div>
                        <select
                            value={filterCustomer}
                            onChange={e => setFilterCustomer(e.target.value)}
                            className="px-4 py-2.5 border border-slate-200 rounded-xl text-slate-700 bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                        >
                ...
                <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-sm border border-slate-200/50 overflow-hidden">
                    {loading ? (
                        ...
                        <div className="text-center py-16">
                            ...
                            <Link
                                href="/projects/create"
                                className="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium rounded-xl hover:from-blue-600 hover:to-indigo-700 shadow-lg shadow-blue-500/30 transition-all"
                            >
                                <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                                </svg>
                                    ...
                                    {filtered.map(p => (
                                        <tr key={p.project_id} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="px-6 py-4">
                                                <span className="text-sm font-semibold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-lg">{p.project_code}</span>
                                            </td>
                                            <td className="px-6 py-4 text-sm font-medium text-slate-800">{p.project_name}</td>
                                            <td className="px-6 py-4 text-sm text-slate-600">{p.customer?.customer_name || '-'}</td>
                                            <td className="px-6 py-4">
                                                <span className={`px-2.5 py-1 rounded-lg text-xs font-medium border ${getStatusStyle(p.status)}`}>
                                                    {p.status}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex gap-2">
                                                    <Link
                                                        href={`/projects/${p.project_id}/edit`}
