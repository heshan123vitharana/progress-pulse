...
export default function KanbanCard({ task }: KanbanCardProps) {
    ...
    return (
        <div className={`bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition cursor-move ${priorityColors[task.priority as keyof typeof priorityColors]}`}>
                ...
                <div className="flex items-center gap-2">
                    {task.employee && (
                        <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs text-gray-600">{task.employee.name.split(' ')[0]}</span>
                        </div>
                    )}
                </div>
                <div className="flex items-center gap-2">
